...
- Env overrides: `MEMORY_DB_PATH`, `OLLAMA_URL`, `QUAID_HOME`, `QUAID_ADAPTER` (path resolution via adapter layer)
...
| `QUAID_ADAPTER` | Force adapter: `standalone` or `openclaw` | Auto-detected |
| `CLAWDBOT_WORKSPACE` | Workspace root (triggers OpenClaw adapter) | Auto-detected from gateway |
