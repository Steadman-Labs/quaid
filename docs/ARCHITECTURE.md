...
| `QUAID_ADAPTER` | Force adapter: `standalone` or `openclaw` (auto-detected if not set) |
| `QUAID_OWNER` | Owner identity for MCP server and CLI (default `"default"`) |
| `CLAWDBOT_WORKSPACE` | Root workspace path (triggers OpenClaw adapter auto-detection) |
...
1. `$QUAID_HOME/config/memory.json` (or `$CLAWDBOT_WORKSPACE/config/memory.json` in OpenClaw mode)
