...
### Smarter Tech Fact Filtering

The extraction prompt currently blanket-rejects "system architecture" and
"infrastructure" facts as noise. This loses genuinely useful technical facts
that aren't tied to an active project — things like "Solomon prefers Postgres
over MySQL" or "decided to use Apache-2.0 for open source projects." The fix:
instead of ignoring all tech facts, filter out only facts that belong to an
active project (those go in project docs, not the memory DB). Unassociated
tech facts — personal technical preferences, decisions, and opinions — should
be extracted and stored like any other personal fact.
- Reclassify "tech noise" as "project-scoped tech" in extraction prompt
- Keep project-specific architecture/config facts out of memory DB
- Extract unassociated technical preferences, decisions, and opinions

### Orphaned Project Discovery

The janitor's auto-discover (task 7b) only scans within already-registered
projects. Folders in `projects/` or `docs/` that don't have a registered
project are invisible. The janitor should scan for unregistered project
directories — any folder under `projects/` containing a `PROJECT.md` (or
matching markdown files) that isn't in the registry should be flagged or
auto-registered.
- Scan `projects/` for directories not in registry
- Scan `docs/` for directories not in registry
- Auto-register or flag for review depending on config
- Run as part of janitor task 7 (RAG/project discovery)
...
`quaid config` currently dumps raw JSON. The config surface is large
(capture, decay, janitor, retrieval, models, docs, projects, confirmation
levels) and growing. Users need a better way to understand and change settings.

Two parts:
- **CLI config menu** — interactive `quaid config edit` that walks through
  settings by category, shows current values and descriptions, validates
  input. Think `git config --interactive` but for Quaid. Replaces manual
  JSON editing for common settings like model selection, decay thresholds,
  confirmation levels, and embedding provider.
- **Web dashboard** — browser-based config viewer at `quaid dashboard`.
  Same local server pattern as the graph visualizer. Read/write access to
  all config sections, live preview of how changes affect behavior (e.g.
  decay curve preview, retrieval parameter tuning). Could share a UI with
  the graph explorer — one dashboard for everything.
...
Users should control how much autonomy Quaid has. Different people want
different levels of oversight — some want to approve every change, others
want a daily summary, others want it fully silent. This applies across the
system, not just core markdown.

Three levels per action category:
- **confirm** — queue changes for explicit approval before applying (notification + CLI review)
- **auto+report** — apply automatically, send a summary of what changed (notification or daily digest)
- **auto+silent** — apply automatically, no notification (current default behavior)

Action categories with independent confirmation levels:
- **Core markdown changes** — SOUL.md, USER.md, MEMORY.md edits from snippet/journal distillation
- **Doc updates** — project documentation auto-updated from git diffs
- **Fact lifecycle** — janitor review decisions (approve/reject/merge), dedup merges, contradiction resolution
- **Memory decay** — facts dropping below confidence threshold

Config example:
```json
"confirmation": {
  "coreMarkdown": "auto+report",
  "docUpdates": "auto+silent",
  "factLifecycle": "auto+report",
  "memoryDecay": "auto+silent"
}
```

Pending changes stored locally, reviewable via `quaid review` CLI or notification.
