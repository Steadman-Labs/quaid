...
async function recall(query, limit = 5, currentSessionId, compactionTime, expandGraph = true, graphDepth = 1, dateFrom, dateTo) {
    try {
        ...
        const agentEndHandler = async (event, ctx) => {
            ...
            inactivityTimer = setTimeout(async () => {
                ...
                try {
                    ...
                    if (log[sid || 'unknown']?.message_count >= msgs.length)
                        return;
                }
                catch { }
...
`);
        ...
        let pendingTimeoutMessages = null;
        let pendingTimeoutSessionId;
        // Shared recall abstraction — used by both memory_recall tool and auto-inject
        async function recallMemories(opts) {
            const { query, limit = 10, expandGraph = false, graphDepth = 1, dateFrom, dateTo, waitForExtraction = false } = opts;
