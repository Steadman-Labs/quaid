...
Rewrite into valid JSON matching the required schema exactly.`;
      ...
      try {
        ...
        throw new Error(
          `Fast recall prepass failed to produce valid structured output after retry (${label}). Use a stronger fast model (e.g. Haiku/GPT-class) or set retrieval.routerFailOpen=true. Last validation error: ${reason2}`
        );
      }
    }
  }
...
intent: ${intent}`;
  ...
  function tokenizeQuery(query) {
    ...
    const tokens = String(query || "").toLowerCase().replace(/[^a-z0-9\s]/g, " ").split(/\s+/).map((t) => t.trim()).filter((t) => t.length >= 3 && !stop.has(t));
