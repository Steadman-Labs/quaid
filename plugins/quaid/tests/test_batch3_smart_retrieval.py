...
class TestClassifyIntent:
    ...
    def test_who_query(self):
        from memory_graph import classify_intent
        intent, boosts = classify_intent("who is Solomon's wife?")
        assert intent == "WHO"
    ...
    def test_when_query(self):
        from memory_graph import classify_intent
        intent, boosts = classify_intent("when is Solomon's birthday?")
        assert intent == "WHEN"
        assert "Event" in boosts

    def test_where_query(self):
        from memory_graph import classify_intent
        intent, boosts = classify_intent("where does Solomon live?")
        assert intent == "WHERE"
        assert "Place" in boosts

    def test_preference_query(self):
        from memory_graph import classify_intent
        intent, boosts = classify_intent("Solomon's favorite food preferences")
        assert intent == "PREFERENCE"
        assert "Preference" in boosts

    def test_relation_query(self):
        from memory_graph import classify_intent
        intent, boosts = classify_intent("how is Levi related to Shannon as siblings")
    ...
    def test_what_query(self):
        from memory_graph import classify_intent
        intent, boosts = classify_intent("what is Solomon working on?")
        assert intent == "WHAT"
...
class TestTemporalValidity:
    ...
    def test_expired_fact_penalized(self):
        from memory_graph import _compute_composite_score, Node
        node = Node.create(type="Fact", name="Solomon lives in Texas")
        ...
        node_current = Node.create(type="Fact", name="Solomon lives in Bali")
    ...
    def test_future_fact_heavily_penalized(self):
        from memory_graph import _compute_composite_score, Node
        node = Node.create(type="Fact", name="Solomon will move to Japan")
        node.accessed_at = datetime.now().isoformat()
        node.valid_from = (datetime.now() + timedelta(days=30)).isoformat()
        score = _compute_composite_score(node, 0.9)
        # Heavy penalty for future facts
        node_current = Node.create(type="Fact", name="Solomon lives in Bali now")
        node_current.accessed_at = datetime.now().isoformat()
        score_current = _compute_composite_score(node_current, 0.9)
    ...
    def test_valid_fact_no_penalty(self):
        from memory_graph import _compute_composite_score, Node
        node = Node.create(type="Fact", name="Solomon is a software engineer")
        node.accessed_at = datetime.now().isoformat()
        ...
        node_no_dates = Node.create(type="Fact", name="Solomon is a software developer")
    ...
    def test_no_temporal_fields_no_penalty(self):
        from memory_graph import _compute_composite_score, Node
        node = Node.create(type="Fact", name="Solomon likes coffee espresso")
        node.accessed_at = datetime.now().isoformat()
        ...
        with patch("memory_graph._lib_get_embedding", side_effect=_fake_get_embedding):
            ...
            with sqlite3.connect(str(graph.db_path)) as conn:
                conn.execute("""
                    ...
                    VALUES ('test-123', 'Fact', 'Solomon likes coffee', 'approved', 'solomon', NULL)
                """)
            ...
            with graph._get_conn() as conn:
                row = conn.execute("SELECT content_hash FROM nodes WHERE id = 'test-123'").fetchone()
                assert row["content_hash"] == _content_hash("Solomon likes coffee")

    def test_backfill_skips_already_hashed(self, tmp_path):
        from memory_graph import Node, _content_hash
        with patch("memory_graph._lib_get_embedding", side_effect=_fake_get_embedding):
            graph = _make_graph(tmp_path)
            node = Node.create(type="Fact", name="Solomon likes espresso coffee", owner_id="solomon")
