...
class TestCamelToSnake:
    ...
    def test_low_reasoning(self):
        assert _camel_to_snake("lowReasoning") == "low_reasoning"
...
class TestModelConfig:
    ...
    def test_context_window_high(self):
        cfg = ModelConfig(high_reasoning_context=200000)
        assert cfg.context_window("high") == 200000

    def test_context_window_low(self):
        cfg = ModelConfig(low_reasoning_context=150000)
        assert cfg.context_window("low") == 150000

    def test_context_window_unknown_tier_defaults_to_low(self):
        cfg = ModelConfig(low_reasoning_context=100000)
        assert cfg.context_window("unknown") == 100000

    def test_default_values(self):
        cfg = ModelConfig()
        assert cfg.low_reasoning == "claude-haiku-4-5"
        assert cfg.high_reasoning == "claude-opus-4-6"
        assert cfg.batch_budget_percent == 0.50

    def test_custom_values(self):
        cfg = ModelConfig(
            low_reasoning="custom-model",
            high_reasoning_context=500000,
            batch_budget_percent=0.30,
        )
        assert cfg.low_reasoning == "custom-model"
        assert cfg.high_reasoning_context == 500000
...
class TestConfigLoading:
    ...
    def test_default_config_when_no_file(self):
        ...
        config._config = None
        try:
            with patch.object(config, "CONFIG_PATHS", [Path("/nonexistent/path.json")]):
                cfg = load_config()
    ...
    def test_load_from_json_file(self, tmp_path):
        ...
        try:
            ...
            with patch.object(config, "CONFIG_PATHS", [config_file]):
                cfg = load_config()
