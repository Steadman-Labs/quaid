...
class TestCheckBloat:
    ...
    def test_under_limit(self, tmp_path):
        ...
        with patch("workspace_audit.get_config", return_value=cfg), \
             patch("workspace_audit.WORKSPACE_DIR", tmp_path):
...
class TestProjectReviewQueue:
    ...
    def test_queue_creates_file(self, tmp_path):
        ...
        from workspace_audit import _queue_project_review, PENDING_PROJECT_REVIEW

        with patch("workspace_audit.DATA_DIR", tmp_path), \
             patch("workspace_audit.PENDING_PROJECT_REVIEW", tmp_path / "pending.json"):
    ...
    def test_queue_appends_to_existing(self, tmp_path):
        ...
        pending_file = tmp_path / "pending.json"
        with patch("workspace_audit.DATA_DIR", tmp_path), \
             patch("workspace_audit.PENDING_PROJECT_REVIEW", pending_file):
    ...
    def test_get_returns_pending(self, tmp_path):
        ...
        with patch("workspace_audit.DATA_DIR", tmp_path), \
            ...
            # File should still exist (non-destructive read)
            assert pending_file.exists()
    ...
    def test_get_returns_empty_for_missing_file(self, tmp_path):
        ...
        with patch("workspace_audit.PENDING_PROJECT_REVIEW", tmp_path / "nope.json"):
    ...
    def test_get_returns_empty_for_corrupt_file(self, tmp_path):
        ...
        with patch("workspace_audit.PENDING_PROJECT_REVIEW", pending_file):
            assert get_pending_project_reviews() == []
