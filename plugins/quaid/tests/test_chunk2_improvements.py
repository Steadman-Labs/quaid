...
class TestRerankerConfig:
    ...
    def test_retrieval_config_reranker_defaults(self):
        ...
        assert cfg.reranker_provider == "anthropic"
        assert cfg.reranker_model == "qwen2.5:7b"  # Only used for ollama provider
    ...
    def test_config_parses_reranker_from_json(self, tmp_path):
        ...
        try:
            with patch("config.CONFIG_PATHS", [config_file]):
                cfg = reload_config()
                assert cfg.retrieval.reranker_enabled is True
                assert cfg.retrieval.reranker_model == "test-model:1b"
...
class TestCrossEncoderReranking:
    ...
    def test_fallback_on_connection_error_ollama(self):
        """_rerank_with_cross_encoder falls back to original scores on connection error (ollama)."""
    ...
    def test_fallback_on_api_error_anthropic(self):
        """_rerank_with_cross_encoder falls back to original scores on API error (anthropic)."""
        from memory_graph import _rerank_with_cross_encoder, Node
        from config import RetrievalConfig
    ...
    def test_anthropic_batches_all_candidates(self):
        """Anthropic provider sends all candidates in a single call."""
        from memory_graph import _rerank_with_cross_encoder, Node
        from config import RetrievalConfig
        ...
        with patch("llm_clients.call_low_reasoning", return_value=(response, 0.5)) as mock_call:
    ...
    def test_anthropic_parses_various_formats(self):
        """Anthropic provider handles different response formats."""
        from memory_graph import _rerank_with_cross_encoder, Node
        from config import RetrievalConfig
        ...
        with patch("llm_clients.call_low_reasoning", return_value=(response, 0.5)):
            reranked = _rerank_with_cross_encoder("test", nodes, config)

        assert len(reranked) == 3

    def test_anthropic_null_response_fallback(self):
        """Anthropic provider falls back when LLM returns None."""
        from memory_graph import _rerank_with_cross_encoder, Node
        from config import RetrievalConfig
        ...
        with patch("llm_clients.call_low_reasoning", return_value=(None, 0.0)):
    ...
    def test_blends_scores_yes_response(self):
        """_rerank_with_cross_encoder blends scores correctly for 'yes' response (ollama)."""
        from memory_graph import _rerank_with_cross_encoder, Node
        from config import RetrievalConfig
    ...
    def test_blends_scores_no_response(self):
        """_rerank_with_cross_encoder blends scores correctly for 'no' response (ollama)."""
        from memory_graph import _rerank_with_cross_encoder, Node
        from config import RetrievalConfig
        ...
        blend = config.reranker_blend  # 0.5 default
        expected = blend * 0.3 + (1 - blend) * original_score
        assert len(reranked) == 1
        assert abs(reranked[0][1] - expected) < 0.001
    ...
    def test_only_reranks_top_k(self):
        ...
        config = RetrievalConfig(reranker_provider="ollama", reranker_top_k=2)
        ...
        # Mock Ollama returning "yes" for all
        mock_response = MagicMock()
        mock_response.__enter__ = MagicMock(return_value=mock_response)
        mock_response.__exit__ = MagicMock(return_value=False)
        mock_response.read.return_value = json.dumps({
        ...
        # Should only call Ollama for top-K=2 items
        assert call_count == 2
        # Should still return all 5 results
        assert len(reranked) == 5

    def test_sorts_by_blended_score(self):
        """_rerank_with_cross_encoder sorts reranked results by blended score (ollama)."""
        from memory_graph import _rerank_with_cross_encoder, Node
        from config import RetrievalConfig
        ...
        call_idx = [0]
        responses = ["no", "yes"]  # First "no" for node_low, "yes" for node_high

        def mock_urlopen(req, timeout=None):
            resp = MagicMock()
            resp.__enter__ = MagicMock(return_value=resp)
            resp.__exit__ = MagicMock(return_value=False)
            answer = responses[call_idx[0]]
            call_idx[0] += 1
