...
"""
    ...
    monkeypatch.setattr(config_mod, "CONFIG_PATHS", [tmp_path / "config" / "memory.json"])
    config_mod.reload_config()

    import docs_registry
    monkeypatch.setattr(docs_registry, "WORKSPACE", tmp_path)

    import project_updater
    monkeypatch.setattr(project_updater, "WORKSPACE", tmp_path)
...
class TestProcessEvent:
    def test_basic_event(self, setup_env):
        ...
        from project_updater import process_event
...
class TestRefreshProjectMd:
    def test_updates_file_list(self, setup_env):
        ...
        from project_updater import refresh_project_md
