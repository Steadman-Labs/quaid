...
describe('Confidence Decay', () => {
  ...
  it('runs decay operation without errors', async () => {
    await memory.store('Test memory for decay', 'default')
  ...
  it('maintains pinned memories during decay', async () => {
    ...
    try {
      ...
      const searchResults = await memory.search('identity fact', 'default')
  ...
  it('maintains verified memories at higher confidence', async () => {
    ...
    try {
      ...
      const searchResults = await memory.search('verified important', 'default')
  ...
  it('processes memories of different owners separately', async () => {
    await memory.store('Quaid memory for decay test', 'default')
    await memory.store('Lori memory for decay test', 'yuni')
    
    await memory.runDecay()
    
    // Both owners' memories should still be accessible
    const ownerResults = await memory.search('decay test', 'default')
  ...
  it('handles multiple decay runs', async () => {
    await memory.store('Multi-decay test memory', 'default')
    ...
    const results = await memory.search('multi-decay', 'default')
    expect(Array.isArray(results)).toBe(true)
  })

  it('preserves memory structure during decay', async () => {
    const original = await memory.store('Structure preservation test', 'default')
    ...
    try {
      ...
      const searchResults = await memory.search('structure preservation', 'default')
  ...
  it('maintains embedding data through decay', async () => {
    const stored = await memory.store('Embedding preservation test', 'default')
    
    await memory.runDecay()
    
    // Should still be semantically searchable
    const results = await memory.search('embedding preservation', 'default')
  ...
  it('handles decay with mixed memory types', async () => {
    await memory.store('Regular fact about testing', 'default', { skipDedup: true })
    await memory.store('Verified fact about testing', 'default', { verified: true, skipDedup: true })
    await memory.store('Pinned fact about testing', 'default', { pinned: true, skipDedup: true })
  ...
  it('reports decay statistics if available', async () => {
    await memory.store('Stats test memory', 'default')
