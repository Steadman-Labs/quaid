...
class TestStoreRecallRoundTrip:
    ...
    def test_store_then_recall_finds_fact(self, tmp_path):
        ...
        with patch("memory_graph.get_graph", return_value=graph), \
            ...
            result = store(
                text="Solomon's favorite color is blue",
                category="preference",
                owner_id="solomon",
                skip_dedup=True,
            )
            ...
            results = recall(
                query="Solomon's favorite color is blue",
                owner_id="solomon",
                limit=5,
                use_routing=False,
                min_similarity=0.0,
    ...
    def test_store_with_edge_creates_both(self, tmp_path):
        ...
        with patch("memory_graph.get_graph", return_value=graph), \
             ...
             patch("memory_graph.route_query", side_effect=lambda q: q):
            # Store two facts
            r1 = store(text="Solomon lives in Bali", owner_id="solomon", skip_dedup=True)
            r2 = store(text="Bali is in Indonesia", owner_id="solomon", skip_dedup=True)
            assert r1 is not None and r2 is not None

            # Create edge (uses subject_name, relation, object_name)
            edge_result = create_edge(
                subject_name="Solomon lives in Bali",
                relation="lives_in",
...
class TestStatusLifecycle:
    ...
    def test_deleted_fact_not_in_recall(self, tmp_path):
        ...
        with patch("memory_graph.get_graph", return_value=graph), \
             ...
             patch("memory_graph.route_query", side_effect=lambda q: q):
            result = store(
                text="Solomon has a cat named Whiskers",
                category="fact",
                owner_id="solomon",
                skip_dedup=True,
            )
            ...
            results = recall(
                query="Solomon has a cat named Whiskers",
                owner_id="solomon",
                limit=5,
                use_routing=False,
                min_similarity=0.0,
    ...
    def test_active_fact_in_recall(self, tmp_path):
        ...
        with patch("memory_graph.get_graph", return_value=graph), \
             ...
             patch("memory_graph.route_query", side_effect=lambda q: q):
            result = store(
                text="Solomon has a dog named Rex",
                category="fact",
                owner_id="solomon",
                skip_dedup=True,
            )
            ...
            results = recall(
                query="Solomon has a dog named Rex",
                owner_id="solomon",
                limit=5,
                use_routing=False,
                min_similarity=0.0,
...
class TestDedupMergeRecall:
    ...
    def test_content_hash_dedup_blocks_exact_duplicate(self, tmp_path):
        ...
        with patch("memory_graph.get_graph", return_value=graph), \
             patch("memory_graph._lib_get_embedding", side_effect=_fake_get_embedding), \
             patch("memory_graph.route_query", side_effect=lambda q: q):
            r1 = store(text="Solomon likes coffee", owner_id="solomon")
            r2 = store(text="Solomon likes coffee", owner_id="solomon")
