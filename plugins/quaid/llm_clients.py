...
def get_api_key(env_var_name: str = None) -> str:
    """Fetch API key via the platform adapter (cached).

    Delegates to adapter.get_api_key() which handles platform-specific
    resolution (env var, .env file, Keychain, etc.).
    ...
    key = get_adapter().get_api_key(env_var_name)
    if key:
        _api_key_cache = key
        return key
...
def _call_via_claude_code(system_prompt: str, user_message: str,
                          model: Optional[str], max_tokens: int,
                          timeout: float) -> Tuple[Optional[str], float]:
    """Route LLM call through Claude Code CLI (uses subscription, not API key).

    Returns (response_text, duration) to match call_llm() signature.
    """
    import os as _cc_os
    ...
    try:
        result = subprocess.run(
            ...
            timeout=max(timeout, 300), env=env,
            cwd="/tmp",  # Avoid loading CLAUDE.md project context
        )
        duration = time.time() - start_time

        if result.returncode != 0:
            print(f"[llm_clients] Claude Code failed (rc={result.returncode}): "
        ...
        for _m, u in data.get("modelUsage", {}).items():
            in_tok = (u.get("inputTokens", 0) + u.get("cacheReadInputTokens", 0)
                      + u.get("cacheCreationInputTokens", 0))
            out_tok = u.get("outputTokens", 0)
            if _m not in _usage_by_model:
                _usage_by_model[_m] = {"input": 0, "output": 0}
            _usage_by_model[_m]["input"] += in_tok
            _usage_by_model[_m]["output"] += out_tok

        return text, duration
    except subprocess.TimeoutExpired:
        duration = time.time() - start_time
        print(f"[llm_clients] Claude Code timed out after {duration:.1f}s", file=sys.stderr)
        return None, duration
    except Exception as e:
        duration = time.time() - start_time
        print(f"[llm_clients] Claude Code error: {e}", file=sys.stderr)
        return None, duration
...
def call_llm(system_prompt: str, user_message: str,
    ...
    # Claude Code dispatch — route through CLI subscription instead of API
    import os as _os2
    if _os2.environ.get("QUAID_USE_CLAUDE_CODE"):
        return _call_via_claude_code(system_prompt, user_message, model, max_tokens, timeout)
