...
`);
        ...
        async function recallMemories(opts) {
            const { query, limit = 10, expandGraph = false, graphDepth = 1, technicalScope = "any", dateFrom, dateTo, waitForExtraction = false, sourceTag = "unknown" } = opts;
            console.log(`[quaid][recall] source=${sourceTag} query="${String(query || "").slice(0, 120)}" limit=${limit} expandGraph=${expandGraph} graphDepth=${graphDepth} technicalScope=${technicalScope} waitForExtraction=${waitForExtraction}`);
