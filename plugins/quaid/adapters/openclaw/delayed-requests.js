...
export function queueDelayedRequest(requestsPath, message, kind = "janitor", priority = "normal", source = "quaid_adapter") {
    try {
...
export function flushDelayedNotificationsToRequestQueue(delayedNotificationsPath, requestsPath, maxItems = 5) {
    let delivered = 0;
    ...
    try {
        ...
        writeJson(delayedNotificationsPath, Object.assign(Object.assign({}, (raw || {})), { items }));
        return { delivered, queuedLlmRequests };
    }
    catch {
...
export function resolveDelayedRequests(requestsPath, ids, resolutionNote = "resolved by agent") {
...
export function clearResolvedRequests(requestsPath) {
    const payload = (readJson(requestsPath) || { version: 1, requests: [] });
    const requests = Array.isArray(payload.requests) ? payload.requests : [];
