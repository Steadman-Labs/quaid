...
function queueDelayedLlmRequest(message: string, kind: string = "janitor", priority: string = "normal"): boolean {
  try {
    ...
    let payload: any = { version: 1, requests: [] };
    ...
    if (!payload || typeof payload !== "object") payload = { version: 1, requests: [] };
...
function flushDelayedNotifications(maxItems: number = 5): { delivered: number; queuedLlmRequests: number } {
  ...
  try {
    if (!fs.existsSync(DELAYED_NOTIFICATIONS_PATH)) return { delivered: 0, queuedLlmRequests };
    const raw = JSON.parse(fs.readFileSync(DELAYED_NOTIFICATIONS_PATH, "utf8"));
    const items = Array.isArray(raw?.items) ? raw.items : [];
    if (!items.length) return { delivered: 0, queuedLlmRequests };
    ...
    return { delivered: sent, queuedLlmRequests };
  } catch (err: unknown) {
    ...
    const beforeAgentStartHandler = async (event: any, ctx: any) => {
      ...
      try {
        ...
        const allMemories = await recallMemories({
          query, limit: autoInjectK, expandGraph: false, technicalScope: "personal", waitForExtraction: false, sourceTag: "auto_inject"
        });
...
dateFrom/dateTo: Use YYYY-MM-DD format to filter memories by date range.`,
        ...
        async execute(toolCallId, params) {
          try {
            ...
            const { query, limit: requestedLimit, expandGraph = true, graphDepth = 1, technicalScope = "personal", dateFrom, dateTo } = params || {};
            ...
            console.log(`[quaid] memory_recall: query="${query?.slice(0, 50)}...", requestedLimit=${requestedLimit}, dynamicK=${dynamicK} (${getActiveNodeCount()} nodes), maxLimit=${maxLimit}, finalLimit=${limit}, expandGraph=${expandGraph}, graphDepth=${depth}, technicalScope=${technicalScope}, dateFrom=${dateFrom}, dateTo=${dateTo}`);
            const results = await recallMemories({
              query, limit, expandGraph, graphDepth: depth, technicalScope,
              dateFrom, dateTo, waitForExtraction: true, sourceTag: "tool"
            });
...
`);
    ...
    async function recallMemories(opts: RecallOptions): Promise<MemoryResult[]> {
      const {
        ...
        graphDepth = 1, technicalScope = "any", dateFrom, dateTo, waitForExtraction = false, sourceTag = "unknown"
      } = opts;

      console.log(
        `[quaid][recall] source=${sourceTag} query="${String(query || "").slice(0, 120)}" limit=${limit} expandGraph=${expandGraph} graphDepth=${graphDepth} technicalScope=${technicalScope} waitForExtraction=${waitForExtraction}`
      );
      ...
      return recall(query, limit, undefined, undefined, expandGraph, graphDepth, technicalScope, dateFrom, dateTo);
    }
...
`);
      ...
      for (const fact of facts) {
        ...
        const storeResult = await store(fact.text, category, sessionId, confNum, resolveOwner(), extractionSource, undefined, undefined, factPrivacy, fact.keywords, knowledgeType, "user", Boolean(fact.is_technical));
