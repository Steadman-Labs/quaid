...
def recall(
    ...
    # Filter out technical/project facts unless the query is project-related.
    # This prevents generic project facts from displacing personal memories in rankings.
    if intent != "PROJECT":
        output = [r for r in output if not r.get("is_technical")]
    ...
    def main():
        ...
        if args.command == "init":
            ...
            if args.archive:
                ...
                results = recall(query, limit=args.limit, owner_id=args.owner, min_similarity=args.min_similarity, current_session_id=args.current_session_id, compaction_time=args.compaction_time, date_from=getattr(args, 'date_from', None), date_to=getattr(args, 'date_to', None), debug=getattr(args, 'debug', False))
            ...
            for r in doc_results:
                ...
                _home_dir = str(get_adapter().quaid_home()) + "/"
                source_short = r["source"].replace(_home_dir, "~/")
