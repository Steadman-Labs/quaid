...
try:
    from llm_clients import call_low_reasoning, parse_json_response
...
class Node:
    ...
    speaker: Optional[str] = None  # Who stated this fact (e.g., "User", "Melina")
...
class MemoryGraph:
    ...
    def get_embedding(self, text: str) -> Optional[List[float]]:
        ...
        model = "unknown"
        try:
            from lib.embeddings import _embedding_model
    ...
    def beam_search_graph(
        ...
        Scoring is adaptive:
          ...
          2. If more candidates than beam_width (truncation needed), the top
             2*beam_width candidates are re-scored via the production Haiku
             reranker in a single batched API call.
        ...
        with self._get_conn() as conn:
            ...
            for depth_level in range(max_depth + 1):
                ...
                if len(next_candidates) > beam_width:
                    ...
                    # Rerank via the production Haiku reranker (single batched call)
    ...
    def _beam_score_candidate(
        ...
        if scoring_mode == "heuristic" or scoring_mode == "hybrid":
            ...
            # LLM-scored: use Haiku to evaluate relevance
            try:
                ...
                response, _ = call_low_reasoning(prompt, max_tokens=10, timeout=5.0)
                if response:
                    import re
...
Example output: ["keyword1", "keyword2", "keyword3"]"""
    ...
    try:
        from llm_clients import call_low_reasoning, parse_json_response
        response, _ = call_low_reasoning(prompt, max_tokens=200, timeout=30)
        if not response:
            return None
...
def _should_expand_graph(query: str) -> bool:
    ...
    # "I'm meeting Melina" - is Melina a Person node?
...
def _resolve_owner_person(owner_id: str) -> Optional[Node]:
    ...
    if not identity or not identity.person_node_name:
        return None

    graph = get_graph()
    return graph.find_node_by_name(identity.person_node_name, type="Person")
...
def _graph_aware_recall(
    ...
    # 2. Vector search (using existing recall) - limit directly, don't multiply
    direct = recall(query, limit=limit, owner_id=owner_id, min_similarity=min_similarity)
...
def _rerank_with_cross_encoder(query: str, results: List[tuple], config_retrieval=None) -> List[tuple]:
    ...
    - "anthropic": Batches all candidates into a single Haiku/Sonnet API call (default).
      Fast, no local memory cost, ~$0.002 per recall.
    - "ollama": Individual calls to a local model (e.g., qwen2.5:7b).
      Fully local but requires ~4.7GB RAM for the model.
    ...
    if config_retrieval:
        provider = getattr(config_retrieval, 'reranker_provider', provider)
        model = getattr(config_retrieval, 'reranker_model', model)
    ...
    if provider == "anthropic":
        reranked = _rerank_via_anthropic(query, to_rerank, model, instruction, config_retrieval)
    else:
        reranked = _rerank_via_ollama(query, to_rerank, model, instruction, config_retrieval)
...
def _rerank_via_anthropic(query: str, candidates: List[tuple], model: str, instruction: str, config_retrieval=None) -> List[tuple]:
    """Batch rerank via Anthropic API — single low-reasoning call for all candidates."""
    from llm_clients import call_low_reasoning
    ...
    try:
        response, _duration = call_low_reasoning(
...
def _rerank_via_ollama(query: str, candidates: List[tuple], model: str, instruction: str, config_retrieval=None) -> List[tuple]:
    """Rerank via individual Ollama calls — one per candidate."""
    ...
    for node, score in candidates:
        try:
            body = json.dumps({
                "model": model,
                "messages": [
                    {"role": "system", "content": f'You are a relevance judge. {instruction}. Answer ONLY "yes" or "no" — is the document relevant to the query?'},
            ...
            with urllib.request.urlopen(req, timeout=5) as resp:
                ...
                rerank_score = 1.0 if "yes" in answer else 0.3
                blended = _blend * rerank_score + (1 - _blend) * score
                reranked.append((node, blended))
        except Exception:
            reranked.append((node, score))
...
def recall(
    ...
    Args:
        ...
        use_routing: Whether to apply HyDE query expansion via Haiku before search
        use_aliases: Whether to resolve entity aliases (e.g., mother → Melina)
    ...
    if _ollama_up:
        # Route query through Haiku (HyDE) — only when embeddings will be used
...
def store(
    ...
    # Dedup check: three-zone logic with optional Haiku verification
    ...
    if not skip_dedup and embedding:
        ...
        if _HAS_CONFIG:
            ...
            haiku_enabled = cfg.janitor.dedup.haiku_verify_enabled
        else:
            ...
            for row in rows:
                ...
                if existing.embedding:
                    ...
                    if sim >= auto_reject_thresh and _texts_are_near_identical(text, existing.name):
                        ...
                        # Zone 2: Gray zone — Haiku verification
                        if haiku_enabled:
                            haiku_result = _haiku_dedup_check(text, existing.name)
                            if haiku_result is not None:
                                if haiku_result["is_same"]:
                                    # Haiku confirms duplicate
                                    _log_dedup_decision(graph, text, existing.id, existing.name,
                                                       sim, "haiku_reject",
                                                       haiku_reasoning=haiku_result.get("reasoning"),
                                    ...
                                    # Haiku says different — log and continue checking
                                    _log_dedup_decision(graph, text, existing.id, existing.name,
                                                       sim, "haiku_accept",
                                                       haiku_reasoning=haiku_result.get("reasoning"),
                                                       owner_id=owner_id, source=source)
                                ...
                                # Haiku unavailable — fall back to legacy threshold
                            ...
                            # Haiku disabled — use legacy threshold
                            # But only reject if texts are near-identical strings
                            if sim >= dedup_threshold and _texts_are_near_identical(text, existing.name):
                                _log_dedup_decision(graph, text, existing.id, existing.name,
                                                   sim, "fallback_reject",
    ...
    # Store source_type in attributes blob
    if source_type:
...
def create_edge(
    ...
    Args:
        subject_name: Name of the source entity (e.g., "Lori")
    ...
    def _find_entity(name: str) -> Optional[Node]:
        ...
        4. Suffix match: last name → full name match (shortest match wins)
        ...
        with graph._get_conn() as conn:
            ...
            row = conn.execute(
                "SELECT * FROM nodes WHERE LOWER(name) = LOWER(?) AND type IN ('Person', 'Place', 'Pet') LIMIT 1",
                (name,)
            ).fetchone()
            if row:
            ...
            if row:
                return graph._row_to_node(row)
            # Suffix match: last name → full name
            row = conn.execute(
                "SELECT * FROM nodes WHERE name LIKE ? AND type IN ('Person', 'Place', 'Pet') ORDER BY LENGTH(name) LIMIT 1",
                ("%" + name,)
            ).fetchone()
    ...
    if not subject and create_missing_entities:
        subject = Node.create(type="Person", name=subject_name)
    ...
    if not obj and create_missing_entities:
        obj = Node.create(type="Person", name=object_name)
...
def _log_dedup_decision(
    ...
    similarity: float,
    decision: str,
    haiku_reasoning: Optional[str] = None,
    owner_id: Optional[str] = None,
    source: Optional[str] = None,
) -> str:
    ...
    with graph._get_conn() as conn:
        conn.execute("""
            ...
            (id, new_text, existing_node_id, existing_text, similarity,
             decision, haiku_reasoning, owner_id, source)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (log_id, new_text, existing_node_id, existing_text,
              similarity, decision, haiku_reasoning, owner_id, source))
    return log_id
...
def _haiku_dedup_check(new_text: str, existing_text: str) -> Optional[Dict[str, Any]]:
    """Ask Haiku whether two texts represent the same fact or different information.
    ...
    prompt = (
        "Are these two statements the same fact (just reworded) or genuinely "
    ...
    response, duration = call_low_reasoning(prompt, max_tokens=100, timeout=5.0)
    if not response:
        return None
...
Write a natural, flowing paragraph. Only include information from the facts above — do not add anything new. Be concise."""
        ...
        try:
            response, _ = call_low_reasoning(prompt)
            if response:
    ...
    def main():
        ...
        if args.command == "init":
            ...
            if args.archive:
                ...
                results = recall(query, limit=args.limit, owner_id=args.owner, min_similarity=args.min_similarity, current_session_id=args.current_session_id, compaction_time=args.compaction_time, debug=getattr(args, 'debug', False))

                # Output format: [similarity] [category][flags][C:confidence] text |ID:id|T:created_at|P:privacy|O:owner_id
                for r in results:
                    flags = []
            ...
            try:
                ...
                if result["status"] == "created":
                    ...
                    print(f"Duplicate (similarity: {result['similarity']}): {result['existing_text'][:80]}")
            ...
            for r in doc_results:
                _home_clawd = str(Path.home() / "clawd") + "/"
                source_short = r["source"].replace(_home_clawd, "~/")
            ...
            query = " ".join(args.query)
            results = recall(query, limit=args.limit, owner_id=args.owner, min_similarity=args.min_similarity, debug=getattr(args, 'debug', False))
            ...
            with graph._get_conn() as conn:
                ...
                if not total_dedup:
                    ...
                    # Sample Haiku reasoning
                    samples = conn.execute("""
                        SELECT new_text, existing_text, similarity, decision, haiku_reasoning
                        FROM dedup_log
                        WHERE haiku_reasoning IS NOT NULL
                        ORDER BY created_at DESC LIMIT 5
                    """).fetchall()
                    if samples:
                        print(f"\nRecent Haiku reasoning samples:")
                        for s in samples:
                            print(f"\n  [{s['decision']}] sim={s['similarity']:.3f}")
                            print(f"  New:      {s['new_text'][:80]}")
                            print(f"  Existing: {s['existing_text'][:80]}")
                            print(f"  Reason:   {s['haiku_reasoning'][:120]}")
