export const RUNTIME_PAIRS = [
  ["adapters/openclaw/index.ts", "adapters/openclaw/index.js"],
  ["adapters/openclaw/command-signals.ts", "adapters/openclaw/command-signals.js"],
  ["adapters/openclaw/delayed-requests.ts", "adapters/openclaw/delayed-requests.js"],
  ["adapters/openclaw/knowledge/orchestrator.ts", "adapters/openclaw/knowledge/orchestrator.js"],
  ["adapters/openclaw/adapter.ts", "adapters/openclaw/adapter.js"],
  ["core/session-timeout.ts", "core/session-timeout.js"],
];

